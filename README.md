...
- **Only inside the workspace** — an argument that names a path leaving the workspace root is still prompted, whether it is absolute, `~/...`, or `../` traversal, and whether it leaves textually or by following a symlink out. What counts as naming a path is the limit: an argument is judged only when it starts with `/` or `~`, or carries a `..` component, so that an ordinary search query is not mistaken for a file. A plain relative argument like `link/secret.txt` is therefore not judged at all, and escapes if `link` is a symlink pointing out of the workspace. A path inside a shell command string is not seen either; see below.
