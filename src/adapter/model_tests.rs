...
/// the "Fetching available models..." banner goes to stderr. Keeps a row in
/// each catalog order: the 2026-09 rename moved the version after `flash`
/// (`gemini-<ver>-flash-*` → `gemini-flash-<ver>-*`). The parser is
/// order-agnostic, so both must parse.
const AGY_MODELS_STDOUT: &str = "\
gemini-3.7-flash-high\tGemini 3.7 Flash (High)
gemini-3.7-flash-low\tGemini 3.7 Flash (Low)
gemini-flash-3.8-low\tGemini 3.8 Flash (Low)
...
fn test_parse_models_output_splits_id_from_label() {
    ...
    assert_eq!(models[2].id, "gemini-flash-3.8-low");
